...
Simple camera device registration:
       ...
       new_camera_device(sensor_name,\       // sensor name, it is equal to CONFIG_SENSOR_X
                          ...
                          flash_attach,\      // sensor is attach flash or not,vaule is true or fase
...
#define CONFIG_SENSOR_FALSH_EN_PIN	  RK30_PIN0_PA3    //high:enable
static int sensor_flash_usr_cb (struct rk29camera_gpio_res *res,int on)
{
	//#error "CONFIG_SENSOR_FLASH_IOCTL_USR is 1, sensor_flash_usr_cb function must be writed!!";
	static int init_flag = 0;
	if(init_flag == 0) {
		//rk30_mux_api_set(GPIO0A3_PWM0_NAME,GPIO0A_GPIO0A3);
		gpio_request(CONFIG_SENSOR_FALSH_EN_PIN,"sensor_flash_pin");
		gpio_direction_output(CONFIG_SENSOR_FALSH_EN_PIN,GPIO_LOW);
		init_flag = 1;
	}
	
	if(on == 0){
		gpio_set_value(CONFIG_SENSOR_FALSH_EN_PIN,GPIO_LOW);
	}else{
		gpio_set_value(CONFIG_SENSOR_FALSH_EN_PIN,GPIO_HIGH);
	}
	return 0;
}
#endif

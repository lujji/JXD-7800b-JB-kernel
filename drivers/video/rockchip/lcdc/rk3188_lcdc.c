...
extern int self_fb_io_init(void);

static struct delayed_work fb_work;

static void enable_lcd_work(struct work_struct *work)
{
...
static int __devinit rk3188_lcdc_probe(struct platform_device *pdev)
{
	...
	if (!first_time){
		first_time=1;
		INIT_DELAYED_WORK(&fb_work,enable_lcd_work);
		schedule_delayed_work(&fb_work,msecs_to_jiffies(2000));
	}

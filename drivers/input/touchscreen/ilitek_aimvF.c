...
//#define ROTATE_FLAG
#define TOUCH_RESET_PIN  RK30_PIN0_PB6	//define reset
...
struct ili2113_platform_data *pdata;
		...
		if(i2c.client->irq != 0 ){ // == => polling mode, != => interrup mode
			...
			if(i2c.irq_work_queue){
				...
				if(request_irq(i2c.client->irq, ilitek_i2c_isr, IRQF_TRIGGER_FALLING , "ilitek_i2c_irq", &i2c)){
					printk(ILITEK_ERROR_LEVEL "%s, request CLOCK irq, error\n", __func__);
				}
				else{
					i2c.valid_irq_request = 1;
					i2c.irq_status = 1;
					printk(ILITEK_ERROR_LEVEL "%s, request CLOCK irq, success\n", __func__);
				}				
				#else
				...
				if(request_irq(i2c.client->irq, ilitek_i2c_isr, IRQF_TRIGGER_LOW, "ilitek_i2c_irq", &i2c)){
					printk(ILITEK_ERROR_LEVEL "%s, request TRIGGER_LOW irq, error\n", __func__);
				}
				else{
					i2c.valid_irq_request = 1;
					i2c.irq_status = 1;
					printk(ILITEK_ERROR_LEVEL "%s, request TRIGGER_LOW irq, success\n", __func__);
				}
				#endif
			}
		}
...
I2C_ERORR:
	...
	if(pdata->exit_platform_hw){
		printk(" ili2113 i2c not responsed\n");
